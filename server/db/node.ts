// server/db/node.ts
// Node.js (better-sqlite3) database implementation.
// NEVER imported by server/worker.ts or any file loaded by the Cloudflare Worker bundle.
// Wrangler aliases server/db/index.ts -> server/db/d1-adapter.ts, so this file
// is excluded from the Worker bundle entirely.
...
// Phase 6.5: add removedAt to roomMembers on databases created before this column existed.
...
// Phase 6.9: conversion metadata columns on uploads
...
// Phase 6.11: backfill persistent room history
...
const backfillRoomHistory = db.transaction(() => {
  ...
  for (const room of rooms) {
    ...
    db.prepare(
      ...
      room.id, room.id, room.code, room.name, room.hostUserId, room.category,
      room.createdAt, room.emptySince, room.emptySince ?? null,
...
export function bootstrapAdminRole(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  for (const email of adminEmails) {
    ...
    db.prepare("UPDATE users SET role = 'admin', updatedAt = ? WHERE id = ?").run(now, row.id);
